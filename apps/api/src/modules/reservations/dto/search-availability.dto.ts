import { z } from 'zod';

/**
 * SearchAvailabilityQuerySchema — validates query params for the availability endpoint.
 *
 * checkIn and checkOut: YYYY-MM-DD strings (parsed as UTC midnight in the controller).
 * adults: coerced from string query param to number.
 */
export const SearchAvailabilityQuerySchema = z.object({
  checkIn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'checkIn must be YYYY-MM-DD'),
  checkOut: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'checkOut must be YYYY-MM-DD'),
  adults: z.coerce.number().int().min(1).max(10),
});

export type SearchAvailabilityQuery = z.infer<typeof SearchAvailabilityQuerySchema>;
