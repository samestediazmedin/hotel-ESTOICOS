 ...
 *
 * The caller uses fire-and-forget:
...
export class EmailService {
   ...
   *
   * Re-throws errors so the cron process can retry.
   */
  async sendPreArrivalReminder(
    ...
    try {
      await this.resend.emails.send({
        from: this.fromEmail,
        to: params.to,
        subject: `Recordatorio: Tu estadía en ${this.escapeHtml(
          params.hotelName,
        )} comienza mañana`,
  ...
  private buildPreArrivalReminderHtml(
    ...
    const specialRequestsHtml = params.specialRequests
      ? `<p style="font-size:14px;color:#5a4d3f;margin:6px 0;"><strong>Solicitudes especiales:</strong> ${this.escapeHtml(
          params.specialRequests,
        )}</p>`
      : '';

    const addressHtml = params.hotelAddress
      ? `<p style="font-size:14px;color:#5a4d3f;margin:6px 0;"><strong>Dirección:</strong> ${this.escapeHtml(
          params.hotelAddress,
        )}</p>`
      : '';

    const phoneHtml = params.hotelPhone
      ? `<p style="font-size:14px;color:#5a4d3f;margin:6px 0;"><strong>Teléfono:</strong> ${this.escapeHtml(
          params.hotelPhone,
        )}</p>`
      : '';
...
<body style="font-family: Geist, Arial, sans-serif; background: #f9f5f0; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="padding: 32px;">
      <p style="font-size: 16px; color: #333; margin-top: 0;">
        Hola, <strong>${this.escapeHtml(params.guestName)}</strong>. Te recordamos que tu estadía en <strong>${this.escapeHtml(
          params.hotelName,
        )}</strong> comienza mañana.
      </p>

      <div style="margin: 24px 0; padding: 20px; background: #f4efe6; border-radius: 8px;">
        <h2 style="font-family: 'Instrument Serif', Georgia, serif; font-size: 18px; color: #2a221a; margin: 0 0 16px; font-weight: normal;">
          Detalles de tu reserva
        </h2>

        <p style="font-size:14px;color:#5a4d3f;margin:6px 0;">
          <strong>Check-in:</strong> ${formatDate(params.checkInDate)}
        </p>

        <p style="font-size:14px;color:#5a4d3f;margin:6px 0;">
          <strong>Check-out:</strong> ${formatDate(params.checkOutDate)}
        </p>

        <p style="font-size:14px;color:#5a4d3f;margin:6px 0;">
          <strong>Habitación:</strong> ${this.escapeHtml(params.roomTypeName)}
        </p>

        <p style="font-size:14px;color:#5a4d3f;margin:6px 0;">
          <strong>Noches:</strong> ${params.totalNights}
        </p>
   ...
   *
   * Unlike sendBookingConfirmation, this method re-throws errors.
   */
  ...
  private buildReviewInviteHtml(
    params: ReviewInviteParams,
  ): string {
    const formatDate = (iso: string) => {
      const date = new Date(`${iso}T12:00:00.000Z`);
...
<body style="font-family: Geist, Arial, sans-serif; background: #f9f5f0; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="background: #2a221a; color: #faf7f2; padding: 32px; text-align: center;">
      <h1 style="margin: 0; font-size: 26px; font-weight: normal; font-family: 'Instrument Serif', Georgia, serif; color: #faf7f2;">
    ...
    <div style="padding: 32px;">
      <p style="font-size: 16px; color: #333; margin-top: 0;">
        Hola, <strong>${this.escapeHtml(params.guestName)}</strong>. Esperamos que hayas disfrutado tu estadía en ${this.escapeHtml(
          params.hotelName,
        )} (${formatDate(params.stayDate)}).
      </p>
      ...
      <div style="text-align: center; margin: 32px 0;">
        <a
          ...
          style="display:inline-block; background:#c4623f; color:#faf7f2; padding:12px 28px; border-radius:8px; text-decoration:none; font-family:Geist,Arial,sans-serif; font-size:16px; font-weight:600;"
        >
          Dejar mi reseña
        </a>
      </div>
  ...
  /**
   * Map ContactPreference enum value to Spanish label.
   */
  ...
  private buildPreferencesSection(
    ...
    if (params.guestContactPreference) {
      parts.push(
        `<p style="font-size:14px;color:#5a4d3f;margin:6px 0;">Prefiere contacto por: <strong>${this.formatContactPreference(
          params.guestContactPreference,
        )}</strong></p>`,
      );
    }
    ...
    if (params.guestDietaryRestrictions) {
      parts.push(
        `<p style="font-size:14px;color:#5a4d3f;margin:6px 0;">Restricciones dietarias: <em>${this.escapeHtml(
          params.guestDietaryRestrictions,
        )}</em></p>`,
      );
    }
    ...
    if (params.guestSpecialRequests) {
      parts.push(
        `<p style="font-size:14px;color:#5a4d3f;margin:6px 0;">Solicitudes especiales: <em>${this.escapeHtml(
          params.guestSpecialRequests,
        )}</em></p>`,
      );
    }
    ...
    <div style="padding: 32px;">
      <p style="font-size: 16px; color: #333; margin-top: 0;">
        Hola, <strong>${this.escapeHtml(
          params.guestName,
        )}</strong>. Tu reserva ha sido confirmada. Aquí están los detalles:
      </p>

      <table style="width: 100%; border-collapse: collapse; margin: 24px 0;">
        <tr>
          <td style="padding: 12px 0; border-bottom: 1px solid #f0ebe4; color: #666; font-size: 14px;">
            Número de reserva
          </td>
          <td style="padding: 12px 0; border-bottom: 1px solid #f0ebe4; color: #333; font-size: 14px; font-weight: bold;">
        ...
        <tr>
          ...
          <td style="padding: 12px 0; border-bottom: 1px solid #f0ebe4; color: #333; font-size: 14px;">
        ...
        <tr>
          ...
          <td style="padding: 12px 0; color: #c45a3a; font-size: 16px; font-weight: bold;">
